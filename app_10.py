...
def get_location_by_point(center_point,start_point,end_point):
    ...
    y_series = np.linspace(min_y, max_y, view_board_shape+1, dtype=int)
    for i in range(view_board_shape):
...
def get_state_from_image_bw(im_bgr):
    ...
    """获取棋盘状态"""
    states = init_state(board_shape=view_board_shape)
